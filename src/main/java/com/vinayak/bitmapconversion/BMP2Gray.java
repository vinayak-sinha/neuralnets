 ...
 * February 2, 2009
 *
 * This program reads in any one of a number of BMP formats and outputs a gray scale 32-bit BMP file image.
 *
 * The default input and output files names as test1.bmp and test2.bmp.
 * These names can be overridden from the command line.
 *
 * javac BMP2Gray.java RgbQuad.java
 *
 * Usage: java BMP2Gray <inputfilename.bmp> <outputfilenae.bmp>
 *
 * The code is NOT written to the coding standards! I took the Microsoft C declarations and code and simply moved into the Java language.
 *
 * This code has a lot of magic numbers. I suggest you figure out what they are for and make properly named constants for them (if you are so inclined)
 *
...
// The color table
      ...
      BMP2Gray dibdumper = new BMP2Gray(); // needed to get to the byte swapping methods
...
// there is no color table for this true color image, so write out the pels
         ...
         for (i = bmpInfoHeader_biHeight - 1; i >= 0; --i)    // write over the rows (in the usual inverted format)
            ...
            for (j = 0; j < bmpInfoHeader_biWidth; ++j) // and the columns
               {
               pel = imageArray[i][j];                  // Get one color pel at a time from the image array,
               pel = dibdumper.colorToGrayscale(pel);   // Convert color to gray scale for RGB becomes YYY (all the bytes are the same)
